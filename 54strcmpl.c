...
int strcmp_(const char *string1, const char *string2)
{
	int num1=strlen_(string1), num2=strlen_(string2),i, flag1=0, flag2=0;
	if(num1<=num2)
	{
		for(i=0;i<num1; i++)
		{
			if(string1[i]<='Z' && string1[i]>='A') flag1='a'-'A';
			if(string2[i]<='Z' && string2[i]>='A') flag2='a'-'A';
			if(string1[i]+flag1<string2[i]+flag2)
			{
				return -1;
			}
			if(string1[i]+flag1>string2[i]+flag2)
